...
- Source game data remains split into human-maintainable JSON files under `src/game-data/**`.
- The running app loads a generated bundled catalog from `public/game-data/catalog.sample.json`, not dozens of individual source JSON requests.
- The bundle is generated by `scripts/generate-game-data-bundle.mjs`.
- The bundle contents come from the curated source files referenced by `src/game-data/loaders/sample-manifest.ts`.
- When adding a new item, ability, buff, perk, relic, EOF spec, or similar curated definition, make sure it is included through the manifest/bundle pipeline so it appears in the generated catalog.
- Do not hand-edit `public/game-data/catalog.sample.json`; treat it as generated output.
- `public/game-data/catalog.sample.json` is gitignored and should be recreated by the normal npm lifecycle on dev/build/test.
- If a build/runtime issue suggests missing game data, check the source JSON, the manifest entry, and the bundle generator before adding ad-hoc loading logic.

#### Game-data content vs TypeScript content
- Canonical game-data content should live in JSON, not in TypeScript maps or registries.
- If a file mostly contains curated labels, icon paths, hover summaries, detail lines, short codes, max ranks, wiki URLs, or similar definition metadata, prefer moving that content into the relevant JSON definitions.
- TypeScript should interpret and render curated data, not duplicate it as a second source of truth.
- UI-only orchestration constants may stay in TypeScript when they are not part of canonical game data.
...
- If you add or change curated game data, ensure the change flows through the bundle generation path instead of adding one-off runtime fetches or TS-only registries.
